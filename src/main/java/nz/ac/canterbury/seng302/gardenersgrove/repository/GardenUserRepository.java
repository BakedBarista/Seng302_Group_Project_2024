...
public interface GardenUserRepository extends CrudRepository<GardenUser, Long> {
    ...
    @Query("SELECT p FROM GardenUser p WHERE ((LOWER(p.fname)=LOWER(?1) and LOWER(p.lname)=LOWER(?2))) and p.id!=?3")
    List<GardenUser> findBySearch(String first, String last, Long currentUserId);

    @Query("SELECT p FROM GardenUser p WHERE ((LOWER(p.fname)=LOWER(?1) and p.lname is null) or LOWER(p.email)=LOWER(?1)) and p.id!=?2")
